        ...
        foreach (string filePath in excelFilePath)
        {
            ...
            if (string.IsNullOrEmpty(errorString))
            {
                ...
                else
                {
                    ...
                    {
                        string strName = fileName + "@Config";
                        TableInfo ti = TableAnalyzeHelper.AnalyzeTable(ds.Tables[AppValues.EXCEL_CONFIG_SHEET_NAME], strName, out errorString);
                        if (!string.IsNullOrEmpty(errorString))
                            Utils.LogErrorAndExit(string.Format("错误：解析表格{0}的配置失败\n{1}", strName, errorString));

                        AppValues.TableInfo.Add(ti.TableName, ti);
                    }

                    // 如果有表格导出配置进行解析
                    if (ds.Tables[AppValues.EXCEL_OPTION_SHEET_NAME] != null)
                    {
                        tableInfo.TableConfig = TableAnalyzeHelper.GetTableConfig(ds.Tables[AppValues.EXCEL_OPTION_SHEET_NAME], out errorString);
                        if (!string.IsNullOrEmpty(errorString))
                            Utils.LogErrorAndExit(string.Format("错误：解析表格{0}的导出配置失败\n{1}", fileName, errorString));
                    }
                ...
                if (AppValues.ExportLuaFilePath != null)
                {
                    ...
                    if (tableInfo.TableConfig != null && tableInfo.TableConfig.ContainsKey(AppValues.CONFIG_NAME_EXPORT))
                    {
                        ...
                        if (inputParams.Contains(AppValues.CONFIG_PARAM_NOT_EXPORT_ORIGINAL_TABLE)
                            || inputParams.Contains(AppValues.CONFIG_PARAM_NOT_EXPORT_ORIGINAL_TABLE_))
                        {
                            inputParams.Remove(AppValues.CONFIG_PARAM_NOT_EXPORT_ORIGINAL_TABLE);
                            inputParams.Remove(AppValues.CONFIG_PARAM_NOT_EXPORT_ORIGINAL_TABLE_);
                        ...
                        bool isCombineSameField = false;
                        if (inputParams.Contains(AppValues.CONFIG_NAME_EXPORT_COMBINE_FIELD))
                        {
                            inputParams.Remove(AppValues.CONFIG_NAME_EXPORT_COMBINE_FIELD);
                            isCombineSameField = true;
                            Utils.LogWarning("你设置了导出合并相同字段");
                        }
                        ...
                        foreach (string param in inputParams)
                        {
                            ...
                            TableExportToLuaHelper.SpecialExportTableToLua(tableInfo, param, isCombineSameField, out errorString);
                            if (errorString != null)
